...
import { ViewModeDropdown } from '../components/ui/ViewModeDropdown';
import {
  semanticSearch,
  generateAIResponse,
...
export default function Home() {
  ...
  // Handle view mode change (for dropdown)
  const handleViewModeChange = (mode: '3d' | 'table' | 'card') => {
    if (mode === '3d') {
      setSearchParams({});
    } else {
  ...
  return (
    <div className={`h-screen text-os-text-primary-dark font-sans overflow-hidden ${displayMode === '3d' ? '' : 'bg-os-bg-dark'}`}>
      {/* 3D Canvas - FIXED BELOW HEADER (only in 3D mode) */}
      {displayMode === '3d' && (
        <div className="fixed inset-0 top-[56px] z-0">
          {/* Top gradient fade for visual separation from header */}
          <div
      ...
      <div className={`relative z-10 h-screen flex flex-col overflow-hidden ${displayMode === '3d' ? 'pointer-events-none' : ''}`}>
        {/* Unified Header - Semi-transparent with backdrop blur */}
        <header className={`pointer-events-auto flex-shrink-0 sticky top-0 z-30 backdrop-blur-xl border-b border-os-border-dark/50 h-14 ${displayMode === '3d' ? 'bg-os-bg-dark/60' : 'bg-os-bg-dark/80'}`}>
            ...
            <div className="flex items-center justify-between h-full">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center justify-center w-10 h-10 bg-os-surface-dark/50 rounded-lg border border-os-border-dark hover:opacity-80 transition-opacity"
                >
                  <svg
                    width="24"
                    height="24"
                ...
                <div className="flex flex-col justify-center">
                  <span className="text-[10px] md:text-caption font-accent uppercase tracking-wider text-os-text-secondary-dark leading-tight">
                    Design Directory
                  </span>
                  <AnimatePresence mode="wait">
                    <motion.span
                      ...
                      className="text-base md:text-lg font-display font-semibold text-brand-vanilla leading-tight"
                      aria-live="polite"
                    >
